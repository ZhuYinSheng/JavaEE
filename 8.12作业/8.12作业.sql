...
ALTER TABLE tb_emp ADD CONSTRAINT emp_depid_fk FOREIGN KEY(deptid) REFERENCES tb_dept(id);
...
(
    ...
    deptId INT(11),
    salary FLOAT,
    constraint emp1_depid_fk foreign key(deptid) references tb_dept(id)
);
...
ALTER TABLE tb_emp DROP FOREIGN KEY emp_depid_fk;
ALTER TABLE tb_emp ADD CONSTRAINT emp_depid_fk1 FOREIGN KEY(deptid) REFERENCES tb_dept1(id);
...
SELECT f_name FROM fruits f INNER JOIN suppliers s ON f.s_id = s.s_id WHERE f_id = 'a1';
...
SELECT f.*, s.s_name FROM fruits f INNER JOIN suppliers s ON f.s_id = s.s_id ORDER BY s_id;
...
SELECT o.o_date FROM orders o WHERE o_num = 30001;
...
-- 计算每个学生的总成绩
SELECT student.`name`, SUM(grade) FROM score INNER JOIN student ON score.stu_id = student.id GROUP BY student.`name`;

-- 计算每个考试科目的平均成绩
SELECT c_name, AVG(grade) FROM score INNER JOIN student ON score.stu_id = student.id GROUP BY c_name;

-- 查询计算机成绩低于95的学生信息
SELECT student.* FROM score INNER JOIN student ON score.stu_id = student.id WHERE c_name = '计算机' AND grade < 95;
...
SELECT DISTINCT student.id FROM score INNER JOIN student ON score.stu_id = student.id;
-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
SELECT student.`name`, student.department, c_name, grade FROM score INNER JOIN student ON score.stu_id=student.id WHERE `name` LIKE '%张%' OR '%王%';

-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩
SELECT student.`name`, 2020-birth, student.department, c_name, grade from score INNER JOIN student ON score.stu_id=student.id WHERE address LIKE '%湖南%';
...
-- 取得每个部门最高薪水的人员名称
SELECT ename FROM emp WHERE emp.sal in (SELECT max(sal) FROM emp GROUP BY deptno);

-- 哪些人的薪水在部门的平均薪水之上
SELECT * FROM emp a WHERE a.sal > (SELECT avg(b.sal) FROM emp b WHERE a.deptno = b.deptno);

-- 取得部门中(所有人)平均薪水的等级
SELECT deptno,a.avg_sal,s.grade FROM (SELECT avg(sal) avg_sal, deptno FROM emp GROUP BY deptno) a INNER JOIN salgrade s ON a.avg_sal BETWEEN s.losal AND s.hisal;

-- 取得部门中(所有人)薪水的平均等级
SELECT a.deptno, AVG(grade) FROM (SELECT e.sal, e.deptno, s.grade FROM emp e INNER JOIN salgrade s ON e.sal BETWEEN s.losal AND s.hisal) a GROUP BY a.deptno;
